...
export default function MetricGraph({ data, period, loading, error, metric, label, unit, color }: Props) {
  ...
  // Reset zoom when data changes (period switch)
  useEffect(() => {
    setDomain(null);
  ...
  const chartData = useMemo<ChartPoint[]>(
    () => (data ?? []).map((d) => ({ t: new Date(d.bucket_start).getTime(), v: d[metric] })),
    [data, metric],
  );
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      const next = { min: Math.max(full.min, newMin), max: Math.min(full.max, newMax) };
  ...
  const CustomTooltip = ({ active, payload, label: lbl }: {
    ...
    if (!active || !payload?.length || lbl == null) return null;
    return (
      <div className="bg-white/90 backdrop-blur border border-gray-100 rounded-xl px-3 py-2 shadow-xl text-sm pointer-events-none">
        <p className="text-gray-400 text-xs mb-0.5">{formatTime(lbl, period)}</p>
      ...
      >
        {loading && (
          <div className="absolute inset-0 flex items-center justify-center z-10">
            <div className="flex gap-1.5">
              {[0, 1, 2].map((i) => (
                <span key={i} className="block w-2.5 h-2.5 rounded-full bg-amber-400 animate-bounce"
        ...
        {!loading && error && (
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-sm text-red-500 bg-red-50/80 px-4 py-2 rounded-lg border border-red-100">{error}</span>
          </div>
        )}
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={visibleData} margin={{ top: 12, right: 14, left: 0, bottom: 4 }}>
            <defs>
            ...
            <XAxis
              dataKey="t" type="number" scale="time" domain={[visMin, visMax]}
              tickFormatter={(t) => formatTime(t as number, period)}
              tick={{ fontSize: 11, fill: "#9ca3af" }} axisLine={false} tickLine={false} minTickGap={52}
            />
            <YAxis
              tick={{ fontSize: 11, fill: "#9ca3af" }} axisLine={false} tickLine={false}
              tickFormatter={(v) => `${(+v).toFixed(0)}`} width={46} domain={["auto", "auto"]}
            />
            <Tooltip content={<CustomTooltip />} />
            <Area
              type="monotone" dataKey="v" stroke={color} strokeWidth={2.5}
              fill={`url(#grad-${metric})`} dot={false}
      ...
      {!loading && avg !== null && visVals.length > 0 && (
        <div className="flex items-center gap-5 text-sm px-1">
          <span className="text-gray-500">
            Min <strong className="text-gray-800 font-semibold">{Math.min(...visVals).toFixed(1)}{unit}</strong>
          </span>
          <span className="text-gray-500">
            Avg <strong className="text-gray-800 font-semibold">{avg.toFixed(1)}{unit}</strong>
          </span>
          <span className="text-gray-500">
            Max <strong className="text-gray-800 font-semibold">{Math.max(...visVals).toFixed(1)}{unit}</strong>
          </span>
